     ...
     {notes.length > 0 ? (
       <ul>
         {notes.map((note) => (
           ...
           key={note.id}>
             {note.content}{' '}
             <button style={{width:"150px", fontSize:"16px",border:"0",background:"yellowgreen", color:"white",borderRadius:"5px",padding:"10px",margin:"20px 20px 0"}} onClick={() => handleDeleteNote(note.id)}>Delete</button>
           </li>
         ))}
       </ul>
     ) : (
